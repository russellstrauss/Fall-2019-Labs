  ...
  return {
    ...
    init: function init() {
      ...
      d3.csv('../activity_2/baseball_hr_leaders.csv').then(function (dataset) {
        ...
        svg.append('g').attr('class', 'x axis').attr('transform', 'translate(0, 345)').call(d3.axisBottom(yearScale).tickFormat(function (d) {
          return d;
        })); // Label x-axis

        svg.append('text').attr('class', 'label').attr('transform', 'translate(360,390)').text('MLB Season'); // Add y-axis markers in increments of 10
